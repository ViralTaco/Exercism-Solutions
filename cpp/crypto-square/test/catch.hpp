...
struct ratio_string<std::milli> {
    ...
    struct StringMaker<std::chrono::duration<Value, Ratio>> {
        static std::string convert(std::chrono::duration<Value, Ratio> const&
          duration) {
            ReusableStringStream rss;
                ...
                for( u_int m = 0; m < count ; m++ ) {
                    ...
                    if( startsWith( methodName, "Catch_TestCase_" ) ) {
                        ...
                        std::string name = Detail::getAnnotation( cls,
                                                                  "Name", testCaseName );
                        std::string desc = Detail::getAnnotation( cls,
...
namespace Catch
{
    ...
    public:
        ...
        WildcardPattern( std::string const& pattern, CaseSensitive::Choice
        caseSensitivity );
...
namespace Catch {
    ...
    private:
        ...
        // subsequent lines
        void printHeaderString(std::string const& _string, std::size_t indent
        = 0);
...
namespace Catch {
    ...
    public:
        ...
        public:
            ...
            ScopedElement& writeText( std::string const& text, bool indent =
              true );

            template<typename T>
            ScopedElement& writeAttribute( std::string const& name, T const&
            attribute ) {
        ...
        XmlWriter& writeAttribute( std::string const& name, std::string
        const& attribute );

        XmlWriter& writeAttribute( std::string const& name, bool attribute );

        template<typename T>
        XmlWriter& writeAttribute( std::string const& name, T const&
        attribute ) {
            ReusableStringStream rss;
            rss << attribute;
...
std::string StringMaker<Catch::Detail::Approx>::convert(Catch::Detail::Approx
const& value) {
...
namespace Catch {
    ...
    public:
        ...
        void testGroupEnded( std::string const& testSpec, Totals const&
        totals, std::size_t groupIndex, std::size_t groupsCount );
    ...
    private:
        ...
        void runCurrentTest( std::string& redirectedCout, std::string&
        redirectedCerr );
...
namespace Catch {
    ...
    void handleExceptionMatchExpr( AssertionHandler& handler, std::string
    const& str, StringRef const& matcherString  ) {
...
public:
	class iterator {
		...
		auto line() const -> std::string const& { return m_column
		.m_strings[m_stringIndex]; }
		...
		auto indent() const -> size_t {
			auto initial = m_pos == 0 && m_stringIndex == 0 ? m_column
			  .m_initialIndent : std::string::npos;
	...
	public:
		...
		explicit iterator(Column const& column) : m_column(column) {
			...
			assert(m_column.m_initialIndent == std::string::npos || m_column
			.m_width > m_column.m_initialIndent);
			calcLength();
			if (m_len == 0)
...
public:
	...
	class iterator {
		...
		iterator(Columns const& cols, EndTag)
			: m_columns(cols.m_columns),
			m_activeIterators(0) {
	...
	public:
		...
		explicit iterator(Columns const& cols)
			: m_columns(cols.m_columns),
...
namespace detail {
    ...
    public:
        ...
        virtual auto parse( std::string const& exeName, TokenStream const
        &tokens) const -> InternalParseResult  = 0;
    ...
    public:
        ...
        template<typename LambdaT>
        Opt( LambdaT const &ref, std::string const &hint ) : ParserRefImpl(
          ref, hint ) {}
    ...
    struct Parser : ParserBase {
        ...
        auto parse( std::string const& exeName, TokenStream const &tokens )
        const -> InternalParseResult override {
...
namespace Catch {
    ...
    std::string const& Config::getReporterName() const { return m_data
    .reporterName; }

    std::vector<std::string> const& Config::getTestsOrTags() const { return
    m_data.testsOrTags; }
    std::vector<std::string> const& Config::getSectionsToRun() const { return
    m_data.sectionsToRun; }
    ...
    std::string Config::name() const                   { return m_data.name
...
namespace Catch {
    ...
    void formatReconstructedExpression( std::ostream &os, std::string const&
    lhs, StringRef op, std::string const& rhs ) {
...
namespace Catch {
    ...
    public:
        ...
        RedirectedStreams(std::string& redirectedCout, std::string&
        redirectedCerr);
        ~RedirectedStreams();
    private:
...
namespace Catch {
    ...
    OutputRedirect::OutputRedirect(std::string& stdout_dest, std::string&
    stderr_dest) :
...
namespace Catch {
    ...
    public:
        ...
        IStreamingReporterPtr create( std::string const& name, IConfigPtr
...
namespace Catch {
    ...
    public:
        ...
        std::string expandAliases( std::string const& unexpandedTestSpec )
        const override;
...
namespace Catch {
    ...
    namespace {
        ...
        public: // IMutableRegistryHub
            ...
            void registerTagAlias( std::string const& alias, std::string
            const& tag, SourceLineInfo const& lineInfo ) override {
...
namespace Catch {
    ...
    void RunContext::runCurrentTest(std::string & redirectedCout, std::string
    & redirectedCerr) {
        auto const& testCaseInfo = m_activeTestCase->getTestCaseInfo();
        SectionInfo testCaseSection(testCaseInfo.lineInfo, testCaseInfo.name);
...
namespace Catch {
    ...
    auto operator + ( StringRef const& lhs, StringRef const& rhs ) ->
    std::string {
        std::string str;
...
namespace Catch {
    ...
    void TagAliasRegistry::add( std::string const& alias, std::string const&
    tag, SourceLineInfo const& lineInfo ) {
...
namespace Catch {
    ...
    namespace {
        TestCaseInfo::SpecialProperties parseSpecialTag( std::string const&
        tag ) {
            if( startsWith( tag, '.' ) ||
                tag == "!hide" )
...
namespace TestCaseTracking {
    ...
    void SectionTracker::addInitialFilters( std::vector<std::string> const&
      filters ) {
        if( !filters.empty() ) {
    ...
    void SectionTracker::addNextFilters( std::vector<std::string> const&
      filters ) {
        if( filters.size() > 1 )
            m_filters.insert( m_filters.end(), ++filters.begin(), filters.end() );
    }
...
namespace Catch {
    ...
    TestSpec::TagPattern::TagPattern( std::string const& tag ) : m_tag(
      toLower( tag ) ) {}
...
namespace Catch {
    ...
    std::string TestSpecParser::subString() const { return m_arg.substr(
      m_start, m_pos - m_start ); }
...
namespace {
    ...
    XmlWriter::ScopedElement XmlWriter::scopedElement( std::string const&
    name ) {
    ...
    XmlWriter& XmlWriter::writeAttribute( std::string const& name, bool
    attribute ) {
...
public:
    ...
    void print() {
        ...
        case ResultWas::ExpressionFailed:
            if (result.isOk())
                printResultType(Colour::ResultSuccess, failedString() +
...
private:
    ...
    void printResultType(Colour::Code colour, std::string const& passOrFail)
    const {
        if (!passOrFail.empty()) {
            {
                Colour colourGuard(colour);
...
void ConsoleReporter::printTotals( Totals const& totals ) {
    if (totals.testCases.total() == 0) {
        ...
        std::vector<SummaryColumn> cols;
        cols.push_back(SummaryColumn("", Colour::None)
                          .addRow(totals.testCases.total())
                          .addRow(totals.assertions.total()));
        cols.push_back(SummaryColumn("passed", Colour::Success)
                          .addRow(totals.testCases.passed)
                          .addRow(totals.assertions.passed));
        cols.push_back(SummaryColumn("failed", Colour::ResultError)
                          .addRow(totals.testCases.failed)
                          .addRow(totals.assertions.failed));
        cols.push_back(SummaryColumn("failed as expected", Colour::ResultExpectedFailure)
                          .addRow(totals.testCases.failedButOk)
                          .addRow(totals.assertions.failedButOk));

        printSummaryRow("test cases", cols, 0);
        printSummaryRow("assertions", cols, 1);
    }
}
...
void ConsoleReporter::printTotalsDivider(Totals const& totals) {
    if (totals.testCases.total() > 0) {
        ...
        if (totals.testCases.allPassed())
            stream << Colour(Colour::ResultSuccess) << std::string
            (passedRatio, '=');
        else
            stream << Colour(Colour::Success) << std::string(passedRatio, '=');
    } else {
        stream << Colour(Colour::Warning) << std::string
    ...
    namespace {
        ...
        std::string fileNameTag(const std::vector<std::string> &tags) {
            auto it = std::find_if(begin(tags),
                                   ...
                                   [] (std::string const& tag) {return tag
                                   .front() == '#'; });
            if (it != tags.end())
                return it->substr(1);
            return std::string();
